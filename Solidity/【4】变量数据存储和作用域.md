...
![example](https://www.wtf.academy/assets/images/5-4-5ee425310e1666a20575f2a8330c55cb.png)
...
- 使用 storage：当需要存储持久化数据时，例如合约状态变量。
- 使用 memory：当需要临时存储数据时，例如函数内部的变量或返回值。
- 使用 calldata：当处理函数调用的输入参数时，尤其是当参数是数组或复杂数据结构时，使用 calldata 可以节省成本。